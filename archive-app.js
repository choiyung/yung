...
function tick(now){const dt=Math.min(now-last||16.7,40);last=now;if(!paused&&!drag&&width&&!document.hidden&&!$(".overlay.is-open")){if(Math.abs(velocity)>.05){offset+=velocity*dt/16.7;velocity*=Math.pow(MOTION.momentumFriction,dt/16.7)}else if(now>pauseUntil)offset+=(mobile.matches?MOTION.mobileAutoSpeed:MOTION.desktopAutoSpeed)*dt/16.7;norm();paint()}requestAnimationFrame(tick)}requestAnimationFrame(tick);
...
const overlays=[...document.querySelectorAll(".overlay")];function close(){overlays.forEach(x=>{x.classList.remove("is-open");x.setAttribute("aria-hidden","true")})}
document.querySelectorAll("[data-open]").forEach(b=>b.onclick=()=>{const x=document.getElementById(b.dataset.open);close();x.classList.add("is-open");x.setAttribute("aria-hidden","false");x.querySelector("input,button,a")?.focus()});document.querySelectorAll(".overlay-close").forEach(b=>b.onclick=close);document.addEventListener("keydown",e=>{if(e.key==="Escape")close()});
document.querySelectorAll(".menu-links a").forEach(a=>a.onclick=e=>{if(a.dataset.route==="archive"){e.preventDefault();close();changeMode("index")}});
